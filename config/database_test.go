...
func TestDatabaseStoreLoad(t *testing.T) {
	...
	t.Run("fixes required", func(t *testing.T) {
		_, tearDown := setupConfigDatabase(t, trailingSlashConfig)
		defer tearDown()
		...
		err = ds.Load()
		require.NoError(t, err)
		assertDatabaseNotEqualsConfig(t, trailingSlashConfig)
...
func TestDatabaseStoreString(t *testing.T) {
	...
	assert.Equal(t, fmt.Sprintf("%s://%s", *mainHelper.Settings.DriverName, *mainHelper.Settings.DataSource), ds.String())
}
